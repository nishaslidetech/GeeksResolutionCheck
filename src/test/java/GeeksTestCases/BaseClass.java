		...
		if (sizeofPagination.size() > 0) {
			...
			for (int j = 1; j < 6; j++) {
				...
				for (int i = 0; i < listofImages.size(); i++) {
					...
					assertTrue(df.format(roundedValue).equals("1.76") || df.format(roundedValue).equals("1.75")
		...
		if (sizeofPagination.size() > 0) {
			...
			for (int j = 1; j < 6; j++) {
				...
				for (WebElement e : listofImages) {
					float width = e.getSize().getWidth();
					float hight = e.getSize().getHeight();
					/*
					 * float width = listofImages.get(i).getSize().getWidth(); float hight =
					 * listofImages.get(i).getSize().getHeight();
					 */

					// System.out.println(e.getAttribute("title") + " -" + width + "-" + hight);
